...
export class WorkshopFormComponent extends FormPageComponent<PageComponentData> implements OnInit {
  ...
  public constructor(
    ...
    private readonly warehouseFormService: WorkshopFormService,
  ...
  public ngOnInit(): void {
    ...
    forkJoin(observables).subscribe((results) => {
      const countries: LabelValue<string>[] = results['0'];
      this.areaBinTypes = this.getBinTypeArray(results['1']);
      this.storageBinTypes = this.getBinTypeArray(results['2']);
  ...
  public loadSuppliyingWarehouses(): void {
    ...
    this.warehouseFormService.fillListSupplyingWarehouses(this.bidtWarehouseDTO.bidtSiteId).subscribe((result) => {
      if (result && result.length) {
        this.supplyingWarehouses = result;
      }
    });
  }
  ...
  public saveWarehouse(): void {
    ...
    this.bidtWarehouseDTO.whCategory = AWPropertiesConstants.WAREHOUSE_CATEGORY_WORKSHOP;
