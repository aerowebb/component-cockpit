...
diff_match_patch.prototype.diff_compute_ = function(a, b, c, d) {
  ...
  return -1 != g
    ? ((c = [
      ...
      c)
    : 1 == f.length
    ? [new diff_match_patch.Diff(DIFF_DELETE, a), new diff_match_patch.Diff(DIFF_INSERT, b)]
...
diff_match_patch.prototype.diff_bisect_ = function(a, b, c) {
  for (
    var d = a.length, e = b.length, f = Math.ceil((d + e) / 2), g = 2 * f, h = Array(g), l = Array(g), k = 0;
    k < g;
    k++
  )
    (h[k] = -1), (l[k] = -1);
  ...
  for (var m = 0 != k % 2, p = 0, x = 0, w = 0, q = 0, t = 0; t < f && !(new Date().getTime() > c); t++) {
    ...
    for (v = -t + w; v <= t - q; v += 2) {
      ...
      if (u > d) q += 2;
        ...
        !m &&
        ((n = f + k - v), 0 <= n && n < g && -1 != h[n] && ((r = h[n]), (y = f + r - n), (u = d - u), r >= u))
      )
  ...
  function c(a) {
    for (var b = '', c = 0, g = -1, h = d.length; g < a.length - 1; ) {
      ...
      : void 0 !== e[l])
        ...
        : (h == f && ((l = a.substring(c)), (g = a.length)), (b += String.fromCharCode(h)), (e[l] = h), (d[h++] = l));
      c = g + 1;
    }
    return b;
  }
...
diff_match_patch.prototype.diff_halfMatch_ = function(a, b) {
  function c(a, b, c) {
    for (
      var d = a.substring(c, c + Math.floor(a.length / 4)), e = -1, g = '', h, k, l, m;
      -1 != (e = b.indexOf(d, e + 1));
...
diff_match_patch.prototype.diff_cleanupSemantic = function(a) {
  for (var b = !1, c = [], d = 0, e = null, f = 0, g = 0, h = 0, l = 0, k = 0; f < a.length; )
    a[f][0] == DIFF_EQUAL
    ...
    if (a[f - 1][0] == DIFF_DELETE && a[f][0] == DIFF_INSERT) {
      ...
      if (d >= e) {
        if (d >= b.length / 2 || d >= c.length / 2)
          a.splice(f, 0, new diff_match_patch.Diff(DIFF_EQUAL, c.substring(0, d))),
            (a[f - 1][1] = b.substring(0, b.length - d)),
            (a[f + 1][1] = c.substring(d)),
            f++;
      } else if (e >= b.length / 2 || e >= c.length / 2)
        a.splice(f, 0, new diff_match_patch.Diff(DIFF_EQUAL, b.substring(0, e))),
          (a[f - 1][0] = DIFF_INSERT),
...
diff_match_patch.prototype.diff_cleanupEfficiency = function(a) {
  for (var b = !1, c = [], d = 0, e = null, f = 0, g = !1, h = !1, l = !1, k = !1; f < a.length; )
    a[f][0] == DIFF_EQUAL
      ...
      : (a[f][0] == DIFF_DELETE ? (k = !0) : (l = !0),
        e &&
          ((g && h && l && k) || (e.length < this.Diff_EditCost / 2 && 3 == g + h + l + k)) &&
          (a.splice(c[d - 1], 0, new diff_match_patch.Diff(DIFF_DELETE, e)),
          (a[c[d - 1] + 1][0] = DIFF_INSERT),
          d--,
          (e = null),
          g && h ? ((l = k = !0), (d = 0)) : (d--, (f = 0 < d ? c[d - 1] : -1), (l = k = !1)),
          (b = !0))),
      f++;
  b && this.diff_cleanupMerge(a);
};
          ...
          ? (0 !== c &&
              ...
              0 !== g &&
                (0 < b - c - d && a[b - c - d - 1][0] == DIFF_EQUAL
                  ...
                  : (a.splice(0, 0, new diff_match_patch.Diff(DIFF_EQUAL, f.substring(0, g))), b++),
                (f = f.substring(g)),
                (e = e.substring(g))),
            ...
            (b -= c + d),
            a.splice(b, c + d),
            e.length && (a.splice(b, 0, new diff_match_patch.Diff(DIFF_DELETE, e)), b++),
            f.length && (a.splice(b, 0, new diff_match_patch.Diff(DIFF_INSERT, f)), b++),
            b++)
...
diff_match_patch.prototype.diff_prettyHtml = function(a) {
  for (var b = [], c = /&/g, d = /</g, e = />/g, f = /\n/g, g = 0; g < a.length; g++) {
    ...
    switch (h) {
      case DIFF_INSERT:
        b[g] = '<ins style="background-color:yellow;">' + l + '</ins>';
        break;
      case DIFF_DELETE:
        b[g] = '<del style="background-color:yellow;">' + l + '</del>';
        break;
      case DIFF_EQUAL:
        b[g] = '<span>' + l + '</span>';
    }
...
diff_match_patch.prototype.match_bitap_ = function(a, b, c) {
  ...
  for (var k, m, p = b.length + a.length, x, w = 0; w < b.length; w++) {
    k = 0;
    for (m = p; k < m; ) d(w, c + m) <= g ? (k = m) : (p = m), (m = Math.floor((p - k) / 2 + k));
    p = m;
    k = Math.max(1, c - m + 1);
    ...
    for (m[q + 1] = (1 << w) - 1; q >= k; q--) {
      ...
      m[q] =
        0 === w ? ((m[q + 1] << 1) | 1) & t : (((m[q + 1] << 1) | 1) & t) | ((x[q + 1] | x[q]) << 1) | 1 | x[q + 1];
    ...
    switch (k) {
      ...
      case DIFF_EQUAL:
        m.length <= 2 * this.Patch_Margin && e && b.length != l + 1
          ? ((a.diffs[e++] = b[l]), (a.length1 += m.length), (a.length2 += m.length))
...
diff_match_patch.prototype.patch_apply = function(a, b) {
  ...
  for (var d = 0, e = [], f = 0; f < a.length; f++) {
    ...
    if (h.length > this.Match_MaxBits) {
      ...
      -1 != k &&
        ((l = this.match_main(b, h.substring(h.length - this.Match_MaxBits), g + h.length - this.Match_MaxBits)),
        -1 == l || k >= l) &&
        (k = -1);
    ...
    if (-1 == k) (e[f] = !1), (d -= a[f].length2 - a[f].length1);
      ...
      ((e[f] = !0),
      (d = k - g),
      (g = -1 == l ? b.substring(k, k + h.length) : b.substring(k, l + this.Match_MaxBits)),
      h == g)
    )
      b = b.substring(0, k) + this.diff_text2(a[f].diffs) + b.substring(k + h.length);
    else if (
      ...
      for (l = 0; l < a[f].diffs.length; l++) {
        ...
        p[0] === DIFF_INSERT
          ...
          : p[0] === DIFF_DELETE && (b = b.substring(0, k + m) + b.substring(k + this.diff_xIndex(g, h + p[1].length)));
...
diff_match_patch.prototype.patch_addPadding = function(a) {
  ...
  0 == e.length || e[e.length - 1][0] != DIFF_EQUAL
    ? (e.push(new diff_match_patch.Diff(DIFF_EQUAL, c)), (d.length1 += b), (d.length2 += b))
    ...
    if (!(a[c].length1 <= b)) {
      ...
      for (var e = d.start1, f = d.start2, g = ''; 0 !== d.diffs.length; ) {
        ...
        for (; 0 !== d.diffs.length && h.length1 < b - this.Patch_Margin; ) {
          ...
          var k = d.diffs[0][1];
          g === DIFF_INSERT
            ? ((h.length2 += k.length), (f += k.length), h.diffs.push(d.diffs.shift()), (l = !1))
            : g === DIFF_DELETE && 1 == h.diffs.length && h.diffs[0][0] == DIFF_EQUAL && k.length > 2 * b
            ? ((h.length1 += k.length),
              (e += k.length),
              (l = !1),
            ...
            : ((k = k.substring(0, b - h.length1 - this.Patch_Margin)),
              (h.length1 += k.length),
              (e += k.length),
              g === DIFF_EQUAL ? ((h.length2 += k.length), (f += k.length)) : (l = !1),
              h.diffs.push(new diff_match_patch.Diff(g, k)),
              k == d.diffs[0][1] ? d.diffs.shift() : (d.diffs[0][1] = d.diffs[0][1].substring(k.length)));
        }
